...
describe('XyzD65', function() {
    ...
    describe('#getX', function() {
        it('returns the x channel', function() {
            ...
            assertClose(color.getX(), 0.7818145658065243);
        });
    });
    ...
    describe('#getY', function() {
        it('returns the y channel', function() {
            ...
            assertClose(color.getY(), 0.8031862396740685);
        });
    });
    ...
    describe('#getZ', function() {
        it('returns the z channel', function() {
            ...
            assertClose(color.getZ(), 1.0182984297418491);
        });
    });
    ...
    describe('#toString', function() {
        it('returns the CSS color string', function() {
            ...
            assert.strictEqual(color.toString(), 'color(xyz-d65 0.78 0.8 1.02)');
        });

        it('includes alpha when needed', function() {
            const color = XyzD65.fromString('rgb(230 230 250 / 50%)');

            assert.strictEqual(color.toString(), 'color(xyz-d65 0.78 0.8 1.02 / 0.5)');
        });
    });

    describe('#withX', function() {
        it('returns a copy with a different x channel', function() {
            const color = XyzD65.fromString('lavender').withX(0.5);

            assert.strictEqual(color.toString(), 'color(xyz-d65 0.5 0.8 1.02)');
        });
    });
    ...
    describe('#withY', function() {
        it('returns a copy with a different y channel', function() {
            ...
            assert.strictEqual(color.toString(), 'color(xyz-d65 0.78 0.5 1.02)');
        });
    });

    describe('#withZ', function() {
        it('returns a copy with a different z channel', function() {
            const color = XyzD65.fromString('lavender').withZ(0.5);

            assert.strictEqual(color.toString(), 'color(xyz-d65 0.78 0.8 0.5)');
        });
    });
});
