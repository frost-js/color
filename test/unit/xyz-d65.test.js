...
import { XyzD65 } from '../../src/index.js';
import { assertClose, assertObjectClose } from '../support/assertions.js';

describe('XyzD65', function() {
    describe('#constructor', function() {
        it('preserves extended channels and clamps alpha', function() {
            const color = new XyzD65(2, -1, 3, 1.5);

            assert.strictEqual(color.toString(), 'color(xyz-d65 2 -1 3)');
        });
    });

    describe('#contrast', function() {
        it('calculates the contrast ratio', function() {
            const color1 = XyzD65.fromString('lavender');
            const color2 = XyzD65.fromString('black');
    ...
    describe('Channel getters', function() {
        it.each([
            ['returns the x channel', 'getX', 0.7818145658065243],
            ['returns the y channel', 'getY', 0.8031862396740685],
            ['returns the z channel', 'getZ', 1.0182984297418491],
        ])('%s', function(_, method, expected) {
            const color = XyzD65.fromString('lavender');
    ...
    describe('#label', function() {
        it('returns the closest CSS color name', function() {
            const color = XyzD65.fromString('lavender').withY(0.5);
    ...
    describe('#space', function() {
        it('returns the color space', function() {
            const color = new XyzD65();

            assert.strictEqual(color.space(), 'xyz-d65');
        });
    });
    ...
    describe('#toString', function() {
        it.each([
            ['returns the CSS color string', 'lavender', 'color(xyz-d65 0.78 0.8 1.02)'],
            ['includes alpha when needed', 'rgb(230 230 250 / 50%)', 'color(xyz-d65 0.78 0.8 1.02 / 0.5)'],
        ])('%s', function(_, input, expected) {
            const color = XyzD65.fromString(input);
    ...
    describe('Channel updates', function() {
        it.each([
            ['returns a copy with a different x channel', 'withX', 0.5, 'color(xyz-d65 0.5 0.8 1.02)'],
            ['returns a copy with a different y channel', 'withY', 0.5, 'color(xyz-d65 0.78 0.5 1.02)'],
            ['returns a copy with a different z channel', 'withZ', 0.5, 'color(xyz-d65 0.78 0.8 0.5)'],
        ])('%s', function(_, method, value, expected) {
            const color = XyzD65.fromString('lavender')[method](value);
