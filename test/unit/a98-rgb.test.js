...
import { A98Rgb } from '../../src/index.js';
...
describe('A98Rgb', function() {
    describe('#constructor', function() {
        it('preserves extended channels and clamps alpha', function() {
            const color = new A98Rgb(2, -1, 3, 1.5);

            assert.strictEqual(color.toString(), 'color(a98-rgb 2 -1 3)');
        });
    });
    ...
    describe('Channel getters', function() {
        it.each([
            ['returns the blue channel', 'getBlue', 0.9765699600542911],
            ['returns the green channel', 'getGreen', 0.8990306743531586],
            ['returns the red channel', 'getRed', 0.8990306743531589],
    ...
    describe('#toString', function() {
        it.each([
            ['returns the CSS color string', 'lavender', 'color(a98-rgb 0.9 0.9 0.98)'],
            ['includes alpha when needed', 'rgb(230 230 250 / 50%)', 'color(a98-rgb 0.9 0.9 0.98 / 0.5)'],
        ])('%s', function(_, input, expected) {
            const color = A98Rgb.fromString(input);
    ...
    describe('Channel updates', function() {
        it.each([
            ['returns a copy with a different blue channel', 'withBlue', 0.5, 'color(a98-rgb 0.9 0.9 0.5)'],
            ['returns a copy with a different green channel', 'withGreen', 0.5, 'color(a98-rgb 0.9 0.5 0.98)'],
            ['returns a copy with a different red channel', 'withRed', 0.5, 'color(a98-rgb 0.5 0.9 0.98)'],
        ])('%s', function(_, method, value, expected) {
            const color = A98Rgb.fromString('lavender')[method](value);
