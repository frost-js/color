...
describe('Srgb', function() {
    ...
    describe('#getBlue', function() {
        it('returns the blue channel', function() {
            ...
            assertClose(color.getBlue(), 0.9803921568627451);
        });
    });
    ...
    describe('#getRed', function() {
        it('returns the red channel', function() {
            ...
            assertClose(color.getRed(), 0.9019607843137255);
        });
    });
    ...
    describe('#toString', function() {
        it('returns the CSS color string', function() {
            ...
            assert.strictEqual(color.toString(), 'color(srgb 0.9 0.9 0.98)');
        });

        it('includes alpha when needed', function() {
            const color = Srgb.fromString('rgb(230 230 250 / 50%)');

            assert.strictEqual(color.toString(), 'color(srgb 0.9 0.9 0.98 / 0.5)');
        });
    });

    describe('#withBlue', function() {
        it('returns a copy with a different blue channel', function() {
            const color = Srgb.fromString('lavender').withBlue(0.5);

            assert.strictEqual(color.toString(), 'color(srgb 0.9 0.9 0.5)');
        });
    });
    ...
    describe('#withGreen', function() {
        it('returns a copy with a different green channel', function() {
            ...
            assert.strictEqual(color.toString(), 'color(srgb 0.9 0.5 0.98)');
        });
    });

    describe('#withRed', function() {
        it('returns a copy with a different red channel', function() {
            const color = Srgb.fromString('lavender').withRed(0.5);

            assert.strictEqual(color.toString(), 'color(srgb 0.5 0.9 0.98)');
        });
    });
});
