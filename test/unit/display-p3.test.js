...
import { DisplayP3 } from '../../src/index.js';
...
describe('DisplayP3', function() {
    describe('#constructor', function() {
        it('preserves extended channels and clamps alpha', function() {
            const color = new DisplayP3(2, -1, 3, 1.5);
    ...
    describe('#contrast', function() {
        it('calculates the contrast ratio', function() {
            const color1 = DisplayP3.fromString('lavender');
            const color2 = DisplayP3.fromString('black');
    ...
    describe('Channel getters', function() {
        it.each([
            ['returns the blue channel', 'getBlue', 0.9737123028250251],
            ['returns the green channel', 'getGreen', 0.9019607843137255],
            ['returns the red channel', 'getRed', 0.9019607843137256],
        ])('%s', function(_, method, expected) {
            const color = DisplayP3.fromString('lavender');
    ...
    describe('#label', function() {
        it('returns the closest CSS color name', function() {
            const color = DisplayP3.fromString('lavender').withGreen(0.5);
    ...
    describe('#toString', function() {
        it.each([
            ['returns the CSS color string', 'lavender', 'color(display-p3 0.9 0.9 0.97)'],
            ['includes alpha when needed', 'rgb(230 230 250 / 50%)', 'color(display-p3 0.9 0.9 0.97 / 0.5)'],
        ])('%s', function(_, input, expected) {
            const color = DisplayP3.fromString(input);
    ...
    describe('Channel updates', function() {
        it.each([
            ['returns a copy with a different blue channel', 'withBlue', 0.5, 'color(display-p3 0.9 0.9 0.5)'],
            ['returns a copy with a different green channel', 'withGreen', 0.5, 'color(display-p3 0.9 0.5 0.97)'],
            ['returns a copy with a different red channel', 'withRed', 0.5, 'color(display-p3 0.5 0.9 0.97)'],
        ])('%s', function(_, method, value, expected) {
            const color = DisplayP3.fromString('lavender')[method](value);
